<template>
  <div
    id="basket"
    class="basket util-bg-bisquit"
    :class="{ 'is-open': basket.basketOpen, 'is-visible': cards.length > 0 || isOccasionInBasket()}"
  >
    <div class="donate-shop__content" v-if="!isPlain">
        ...
        <transition-group name="packages" tag="div" class="basket__package-wrapper">
          <div class="basket__package" v-for="(item, index) in cards" :key="index">
            <div class="basket__package-title">
              {{item.title}}
              <span class="basket__remove" @click="removeItem(index)">
                <img :src="`${baseUrl}icons/close.svg`" />
              </span>
            </div>
            <Input
              :value="item.value"
              @amountChanged="setAmount(item.id, ...arguments)"
              :amount="item.amount"
            />
          </div>
        </transition-group>
        <div class="basket__download" v-if="isOccasionInBasket()" ref="basketOccasion">
          <div class="basket__occasion-title">Grusskarte "{{occasion.title}}"</div>
          <span>PDFs zum Herunterladen</span>
          <img class="basket__remove" @click="removeOccasion()" :src="`${baseUrl}icons/close.svg`" />
        </div>
      </div>
      <div class="basket__sum">
        Gesamtsumme
        <span class="input__value">{{accumulatedValue}}€</span>
      </div>
      <div class="basket__cta">
        <Button
          ...
          @click.native.prevent="$emit('basket-btn-clicked');closeBasket();"
        />
        <Button
          v-else
          :text="'Bezahlen'"
          :isDisabled="cards.length === 0 || !isOccasionInBasket()"
          @click.native.prevent="$emit('basket-btn-clicked');cacheBasket();"
        />
      </div>
    </div>
    ...
    <div class="donate-shop__content" v-else>
      ...
      <div class="basket__content">
        <transition-group name="packages" tag="div" class="basket__package-wrapper">
          <div class="basket__package" v-for="(item, index) in cards" :key="index">
            <div class="basket__package-title">{{item.title}}</div>
